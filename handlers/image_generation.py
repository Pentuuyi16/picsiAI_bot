...
async def generation_model_selection_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"<blockquote>⚠️ Запрещено генерировать:\n"
        f"• Неприемлемый контент (NSFW)\n"
        f"• Изображения известных личностей\n"
        f"• Контент, нарушающий правила Google\n\n"
        f"⚡ При нарушении генерация <b>не списывается</b></blockquote>",
        parse_mode="HTML"
    )
